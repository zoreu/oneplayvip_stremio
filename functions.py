...
def get_base_url():
    """Retorna a URL base fixa do servidor Xtream Codes."""
    host = config['xtream_host']
    if host.endswith('/'):
        host = host[:-1]
    return host
...
def get_user_data(user_conf):
    ...
    if isinstance(retrieved_data, dict):
        base_url = config['xtream_host']  # Usa o host fixo do config
        domain_name = urlparse(base_url).hostname or "unknown"
        id_prefix = "oneplayvip"  # Prefixo fixo
        obj = {
            'baseURL': base_url,
        ...
        return {'result': "Invalid user data"}
...
def get_manifest(user_conf):
    ...
    manifest = {
        'id': "org.community.oneplayvip",
        'version': config['version'],
        'name': f"{config['title']}",
        'logo': config['logo'],
        'description': "OnePlay VIP, tenha sua conta pelo whatsapp +55 31 9594-9574",
        'idPrefixes': [prefix],
...
def get_meta(url, type, id):
    ...
    if type == "movie":
        ...
        try:
            ...
            year = 0
        try:
            genres = str(meta_data['info'].get('genre', ""))
            if genres:
                if ',' in genres:
        ...
        if poster:
            if 'pkweb.ca' in poster:
                poster = 'https://oneplayhd.com/proxy_site/?url=' + str(poster)
        background = meta_data['info'].get('backdrop_path', [""])[0]
        if background:
            if 'pkweb.ca' in background:
                background = 'https://oneplayhd.com/proxy_site/?url=' + str(background)            
        return {
            'id': f"{prefix}:{stream_id}",
            'type': type,
            'name': meta_data['info'].get('name', ""),
            'year': year,
            'poster': get_image_url(poster),
            'background': get_image_url(background),
            'genres': genres,
        ...
        meta = {
            ...
            'background': get_image_url(poster),
            'genres': genres,
            'trailers': [],
        ...
        for season, episodes in meta_data.get('episodes', {}).items():
            for episode in episodes:
                videos.append({
                    ...
                    'season': int(episode.get('season', 0)),  # Converte para int
                    'number': int(episode.get('episode_num', 0)),  # Converte para int
                    'episode': episode.get('episode_num'),
                    'thumbnail': get_image_url(episode.get('info', {}).get('movie_image', ""))
                })

        # Ordenar vídeos por season (ascendente) e number (ascendente)
        meta['videos'] = sorted(videos, key=lambda x: (x['season'], x['number']))                   
        ...
        for item in meta_data:
            if int(item['stream_id']) == int(stream_id):
                return {
                    ...
                    'name': item.get('name', ""),
                    'type': type,
                    'background': get_image_url("https://raw.githubusercontent.com/zoreu/oneplayvip_stremio/refs/heads/main/fanart.jpg"),
